...
export class PublicEvidencePopupComponent implements OnInit {
    faTimes = faTimes;
    @Input() verifiableCredentialName: string = "";
    @Input() verifiableCredentialId: number;
    @Input() publicShareInfo: { shareId: number; hash: string; code: string };
    ...
    getEvidenceData() {
        ...
        const publicShareRequest: PublicShareRequestViewModel = {
            hash: this.publicShareInfo.hash,
            code: this.publicShareInfo.code,
        };

        this.evidenceService
            .getEvidences(this.verifiableCredentialId, this.publicShareInfo.shareId, publicShareRequest)
            .subscribe(data => {
