...
export class LogService {
    ...
    logNgError(error: string): Observable<any> {
        ...
        if (environment.debug) console.log(`LogService.logNgError url:${apiUrl} message:${JSON.stringify(angErr)}`);
        console.error("logNgError", error);
        return this.http
            .post<ApiResponse>(apiUrl, angErr)
            .pipe(catchError(err => this.handleLoggingError(err, "Angular")));
    }
    logUploadError(callerInfo: string) {
        ...
        return this.http.post<string>(apiUrl, { message }, { headers }).pipe(
            take(1),
            map(
                data => {
                    errorId = data;
                ...
                err => {
                    console.error("An error occurred trying to log an Upload Error to the server.", err);
                }
            )
        );
    }
    logHttpError(callerInfo: string, error: HttpErrorResponse) {
        ...
        return this.http.post<string>(apiUrl, { message }, { headers }).pipe(
            ...
            map(
                ...
                err => {
                    console.error(`An error occurred trying to log an Http Error to the server.`, err);
                }
            )
        );
    }
    handleLoggingError(error: Error | HttpErrorResponse, callType: string) {
        console.error("An error occurred trying to log an Error to the server.", callType, error);
        return of();
    }
}
